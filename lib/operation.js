  ...
  _createClass(Operation, null, {
    ...
    giveAccess: {
      ...
      value: function giveAccess(opts) {
        ...
        var giveSignersAccess = new xdr.GiveSignersAccessOp(attributes);
    ...
    setSigners: {
      ...
      value: function setSigners(opts) {
        ...
        var setSigners = new xdr.SetSignersOp(attributes);
      ...
      value: function fromXDRObject(operation) {
        ...
        switch (operation.body()["switch"]().name) {
          ...
          case "setOption":
            ...
            if (attrs.signer()) {
              var _signer = {};
              var _arm = attrs.signer().key().arm();
              if (_arm == "ed25519") {
                _signer.ed25519PublicKey = accountIdtoAddress(attrs.signer().key());
              } else if (_arm == "preAuthTx") {
                _signer.preAuthTx = attrs.signer().key().preAuthTx();
              } else if (_arm == "hashX") {
                _signer.sha256Hash = attrs.signer().key().hashX();
              }

              _signer.weight = attrs.signer().weight();
              result.signer = _signer;
            }
            break;
          case "manageOffer":
